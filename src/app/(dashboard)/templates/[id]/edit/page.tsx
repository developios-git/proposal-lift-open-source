...
export default function EditTemplatePage() {
  ...
  /** Last saved values from the server (persisted fields only). */
  const [initialSnapshot, setInitialSnapshot] =
    useState<TemplateEditSnapshot | null>(null);

  const isDirty = useMemo(() => {
    if (!initialSnapshot) return false;
    return (
      templateName !== initialSnapshot.name ||
      category !== initialSnapshot.category ||
      content !== initialSnapshot.content ||
      description !== initialSnapshot.description
    );
  }, [initialSnapshot, templateName, category, content, description]);
  ...
  const fetchTemplate = async () => {
    ...
    try {
      ...
      const data = await res.json() as { template: { name: string; category: string; content: string; description: string | null } };
      const tmpl = data.template;
      const tmplDescription = tmpl.description ?? "";
      setTemplateName(tmpl.name);
      setCategory(tmpl.category);
      setContent(tmpl.content);
      setDescription(tmplDescription);
      setInitialSnapshot({
        name: tmpl.name,
  ...
  // Declared after `fetchTemplate`, and awaited inside an async IIFE so its
  ...
  useEffect(() => {
    ...
    // Re-runs on the route param alone; `fetchTemplate` closes over setters.
  ...
  const sampleData: Record<string, string> = {
    ...
    custom_hook:
      "We noticed your recent series B funding and believe your current infrastructure needs a robust modernization to handle the 400% traffic increase you're forecasting.",
  ...
  if (loadError && !templateName) {
    return (
      <div className="flex flex-col items-center justify-center h-96 gap-4">
        <AlertCircle className="h-12 w-12 text-destructive" />
        <p className="text-lg font-semibold">Error loading template</p>
